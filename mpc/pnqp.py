...
def pnqp(H, q, lower, upper, x_init=None, n_iter=20):
    ...
    for i in range(n_iter):
        ...
        while max_armijo <= GAMMA and count < 10:
            ...
            # NOTE :: torch.diag() is horribly to use on the GPU as it takes up a lot of memory.
            # Old ct = torch.diag(alpha).mm(dx)
            ct = torch.mul(alpha.unsqueeze(-1), dx) # I do not know what it is, so I gave it a random name
            maybe_x = util.eclamp(x + ct, lower, upper)
