...
@profile
def lqr_backward(ctx, C, c, F, f, n_state, n_ctrl, T, u_lower, u_upper, u_zero_I, delta_u, delta_space, verbose):
    u = ctx.current_u
    Ks = []
    ks = []
    prev_kt = None
...
def lqr_forward(ctx, x_init, C, c, F, f, Ks, ks, T, u_lower, u_upper, u_zero_I, delta_u,
    ...
    i = 0
    while (current_cost is None
           or (old_cost is not None and torch.any(current_cost > old_cost))) and \
        ...
        for t in range(T):
            ...
            ut = u[t]
            dxt = dx[t]
            # Note: torch.diag(alphas).mm(kt) is equivalent to alphas * kt
            ct = alphas.unsqueeze(-1) * kt
            new_ut = util.bmv(Kt, dxt) + ut + ct
            ...
            if t < T - 1:
                if isinstance(true_dynamics, mpc.LinDx):
                    ...
                    # Not much to optimize here as it is not our env, so no point in profiling
        ...
        mask = torch.where(current_cost > old_cost)  # TODO::SLOW!
        alphas[mask] *= linesearch_decay
        i += 1
...
def LQRStep(n_state,
    ...
    class LQRStepFn(Function):
        ...
        def forward(ctx, x_init, C, c, F, f=None):
            ...
            Ks, ks, n_total_qp_iter = lqr_backward(ctx, C, c_back, F, f_back, n_state, n_ctrl, T, u_lower, u_upper,
                                                   u_zero_I, delta_u, delta_space, verbose)
            new_x, new_u, for_out = lqr_forward(ctx,
                                                x_init, C, c, F, f, Ks, ks, T, u_lower, u_upper, u_zero_I, delta_u,
                                                linesearch_decay, max_linesearch_iter, true_cost, true_dynamics)
